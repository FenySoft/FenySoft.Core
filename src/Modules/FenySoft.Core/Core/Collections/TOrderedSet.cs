...
{
    public class TOrderedSet<TKey, TValue> : ITOrderedSet<TKey, TValue>
    {
        ...
        protected TOrderedSet(IComparer<TKey> comparer, IEqualityComparer<TKey> equalityComparer, SortedSet<KeyValuePair<TKey, TValue>> set)
        {
            this.comparer = comparer;
            this.equalityComparer = equalityComparer;
            kvComparer = new KeyValuePairComparer<TKey, TValue>(comparer);
        ...
        protected TOrderedSet(IComparer<TKey> comparer, IEqualityComparer<TKey> equalityComparer, int capacity)
            : this(comparer, equalityComparer, new List<KeyValuePair<TKey, TValue>>(capacity))
        {
        }

        public TOrderedSet(IComparer<TKey> comparer, IEqualityComparer<TKey> equalityComparer)
        ...
        public ITOrderedSet<TKey, TValue> Split(int count)
        {
            if (list != null)
            {
                var right = list.Split(count);

                return new TOrderedSet<TKey, TValue>(Comparer, EqualityComparer, right);
            }
            else
            {
                if (dictionary != null)
        ...
        /// </summary>
        public void Merge(ITOrderedSet<TKey, TValue> set)
        {
            if (set.Count == 0)
                return;
        ...
        #region ITOrderedSet<TKey,TValue> Members
