namespace FenySoft.Core.Extensions
{
    ...
    {
        public static IEnumerable<T> Merge<T>(this IEnumerable<T> collection1, IEnumerable<T> collection2, IComparer<T> comparer, TOnMergeConflict onConflict = TOnMergeConflict.ReturnFirstAndSecond, Func<T, T, T> function = null)
        {
            ...
            if (haveNext1 && haveNext2)
            {
                ...
                while (true)
                {
                    ...
                    else
                    {
                        switch (onConflict)
                        {
                            case TOnMergeConflict.ReturnFirstAndSecond:
                                {
                                    yield return item1;
                                    yield return item2;
                                }
                            ...
                            case TOnMergeConflict.ReturnFirst:
                                {
                                    yield return item1;
                                }
                                break;

                            case TOnMergeConflict.ReturnSecond:
                                {
                                    yield return item2;
                                }
                                break;

                            case TOnMergeConflict.ReturnByFunction:
                                {
                                    yield return function(item1, item2);
                                }
                                break;

                            //case TOnMergeConflict.Skip:
                            //    {
                            //    }
                            //    break;
                        }
        ...
        public static IEnumerable<T> Merge<T>(this IEnumerable<T> enumerable1, IEnumerable<T> enumerable2, TOnMergeConflict onMergeConflict = TOnMergeConflict.ReturnFirstAndSecond, Func<T, T, T> function = null)
        {
