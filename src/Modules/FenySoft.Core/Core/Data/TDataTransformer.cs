...
{
    public class TDataTransformer<T> : ITTransformer<T, ITData>
    {
        public readonly Func<T, ITData> to;
        public readonly Func<ITData, T> from;
        ...
        public TDataTransformer(Type type2, Func<Type, MemberInfo, int> membersOrder1 = null, Func<Type, MemberInfo, int> membersOrder2 = null)
        {
        ...
        public Expression<Func<T, ITData>> CreateToMethod()
        {
            var value = Expression.Parameter(Type1);
            var data = Expression.Variable(typeof(TData<>).MakeGenericType(Type2));

            List<Expression> list = new List<Expression>();
            if (TransformerHelper.IsEqualsTypes(Type1, Type2))
                list.Add(Expression.Label(Expression.Label(typeof(ITData)), Expression.New(data.Type.GetConstructor(new Type[] { Type1 }), value)));
            else
            {
            ...
            return Expression.Lambda<Func<T, ITData>>(TransformerHelper.IsEqualsTypes(Type1, Type2) ? list[0] : Expression.Block(typeof(ITData), new ParameterExpression[] { data }, list), value);
        }

        public Expression<Func<ITData, T>> CreateFromMethod()
        {
            var idata = Expression.Parameter(typeof(ITData));
            var data = Expression.Variable(typeof(TData<>).MakeGenericType(Type2));
            ...
            return Expression.Lambda<Func<ITData, T>>(TransformerHelper.IsEqualsTypes(Type1, Type2) ? list[0] : Expression.Block(Type1, new ParameterExpression[] { data, value }, list), idata);
        }


        public ITData To(T value1)
        {
            return to(value1);
        }

        public T From(ITData value2)
        {
            return from(value2);
        }
    }
    ...
    //public class TickSlotesTransformer : ITransforemer<Tick, ITData>
    //{
    //    public ITData To(Tick value1)
    //    {
    //        TData<Slotes> dataSlotes = new TData<Slotes>();
    ...
    //    public Tick From(ITData value2)
    //    {
    //        TData<Slotes> dataSlotes = (TData<Slotes>)value2;
    ...
    //public class TickTickNewTranformer : ITransforemer<Tick, ITData>
    //{
    //    public ITData To(Tick value1)
    //    {
    //        TData<TickNew> dataTickNew = new TData<TickNew>();
    ...
    //    public Tick From(ITData value2)
    //    {
    //        TData<TickNew> dataTickNew = (TData<TickNew>)value2;
    ...
    //public class TickTickTranformer : ITransforemer<Tick, ITData>
    //{
    //    public ITData To(Tick value1)
    //    {
    //        return new TData<Tick>(value1);
    //    }

    //    public Tick From(ITData value2)
    //    {
    //        return ((TData<Tick>)value2).Value;
    //    }
    //}

    //public class BarBarNewTransformer : ITransforemer<Bar, ITData>
    //{
    //    public ITData To(Bar value1)
    //    {
    //        TData<BarNew> dataBarNew = new TData<BarNew>();
    ...
    //    public Bar From(ITData value2)
    //    {
    //        TData<BarNew> dataBarNew = (TData<BarNew>)value2;
    //        Bar bar;
