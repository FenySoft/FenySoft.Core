...
{
    public class TTransformer<T1, T2> : ITTransformer<T1, T2>
    {
        ...
        public TTransformer(Func<Type, MemberInfo, int> membersOrder1 = null, Func<Type, MemberInfo, int> membersOrder2 = null)
        {
    ...
    public static class TransformerHelper
    {
        public static Expression BuildBody(Expression Value1, Expression Value2, Func<Type, MemberInfo, int> membersOrder1, Func<Type, MemberInfo, int> membersOrder2)
        {
                ...
                return Expression.Assign(Value1,
                    Expression.New((typeof(KeyValuePair<,>).MakeGenericType(key.Type, value.Type)).GetConstructor(new Type[] { key.Type, value.Type }),
                        Expression.Block(key.Type,
                            ...
                            BuildBody(key, Expression.PropertyOrField(Value2, type2.IsKeyValuePair() ? "Key" : TDataTypeUtils.GetPublicMembers(Value2.Type, membersOrder2).First().Name), membersOrder1, membersOrder2),
                            Expression.Label(Expression.Label(key.Type), key)),
                        Expression.Block(value.Type,
                            new ParameterExpression[] { value },
                            BuildBody(value, Expression.PropertyOrField(Value2, type2.IsKeyValuePair() ? "Value" : TDataTypeUtils.GetPublicMembers(Value2.Type, membersOrder2).Last().Name), membersOrder1, membersOrder2),
            ...
            {
                if (!TDataType.IsPrimitiveType(type1.GetGenericArguments()[0]) && type1.GetGenericArguments()[0] == typeof(Guid))
            ...
            if (type1.IsNullable())
            {
                ...
                var constructParam = Expression.PropertyOrField(data2Var, type2.IsNullable() ? "Value" : TDataTypeUtils.GetPublicMembers(type2, membersOrder2).First().Name);
            ...
            if (type1.IsClass || type1.IsStruct())
            {
                ...
                List<MemberInfo> members1 = TDataTypeUtils.GetPublicMembers(Value1.Type, membersOrder1).ToList();
                ...
                if (type2.IsKeyValuePair() || type2.IsNullable())
                {
                    ...
                    members2 = TDataTypeUtils.GetPublicMembers(Value2.Type, membersOrder2).ToList();
        ...
        public static bool CheckCompatible(Type type1, Type type2, HashSet<Type> cycleCheck, Func<Type, MemberInfo, int> membersOrder1 = null, Func<Type, MemberInfo, int> membersOrder2 = null)
        {
            ...
            if (TDataType.IsPrimitiveType(type1))
            ...
            if (type1.IsClass || type1.IsStruct())
            {
                ...
                foreach (var slote in TDataTypeUtils.GetPublicMembers(type1, membersOrder1))
                    type1Slotes.Add(slote.GetPropertyOrFieldType());

                foreach (var slote in TDataTypeUtils.GetPublicMembers(type2, membersOrder2))
