...
namespace FenySoft.Core.Data
{
    public class TComparer<T> : IComparer<T>
    {
        ...
        public TComparer(TCompareOption[] compareOptions, Func<Type, MemberInfo, int> membersOrder = null)
        {
            Type = typeof(T);
        ...
        public TComparer(Func<Type, MemberInfo, int> memberOrder = null)
            : this(TCompareOption.GetDefaultCompareOptions(typeof(T), memberOrder), memberOrder)
        {
        }
    ...
    {
        public static Expression CreateComparerBody(List<Expression> expressions, List<ParameterExpression> parameters, Expression x, Expression y, TCompareOption[] compareOptions, Func<Type, MemberInfo, int> membersOrder)
        {
            ...
            if (TDataType.IsPrimitiveType(x.Type) || x.Type == typeof(Guid))
            ...
            else
            {
                ...
                foreach (var field in TDataTypeUtils.GetPublicMembers(x.Type, membersOrder))
                {
                    ...
                    foreach (var command in GetCompareCommands(Expression.PropertyOrField(x, field.Name), Expression.PropertyOrField(y, field.Name), cmp, exitPoint, field.GetPropertyOrFieldType(), compareOptions[i++], i == TDataTypeUtils.GetPublicMembers(x.Type, membersOrder).Count()))
                        list.Add(command);
                }
            }
        ...
        private static IEnumerable<Expression> GetCompareCommands(Expression x, Expression y, ParameterExpression cmp, LabelTarget exitPoint, Type type, TCompareOption compareOption, bool isLastCompare)
        {
            var field1 = x;
            var field2 = y;
            ...
            else if (type == typeof(Decimal))
            {
                ...
                if (!isLastCompare)
                {
                    //cmp = TComparer<T>.Default.Compare(field1, field2);
                    //if (cmp != 0)
                    //    return cmp;
                ...
                {
                    //return TComparer<T>.Default.Compare(field1, field2);
