...
namespace FenySoft.Core.Data
{
    public class TEqualityComparer<T> : IEqualityComparer<T>
    {
        ...
        public TEqualityComparer(TCompareOption[] compareOptions, Func<Type, MemberInfo, int> membersOrder = null)
        {
            TCompareOption.CheckCompareOptions(typeof(T), compareOptions, membersOrder);
            Type = typeof(T);
        ...
        public TEqualityComparer(Func<Type, MemberInfo, int> membersOrder = null)
            : this(TCompareOption.GetDefaultCompareOptions(typeof(T), membersOrder), membersOrder)
        {
        }
    ...
    {
        public static Expression CreateEqualsBody(Expression x, Expression y, TCompareOption[] compareOptions, Func<Type, MemberInfo, int> membersOrder)
        {
            var type = x.Type;
            var exitPoint = Expression.Label(typeof(bool));
            ...
            else
            {
                ...
                int i = 0;
                int count = TDataTypeUtils.GetPublicMembers(type, membersOrder).Count();
        ...
        private static Expression GetEqualsCommand(Expression x, Expression y, TCompareOption compareOption, LabelTarget exitPoint, bool isLast)
        {
            var type = x.Type;
