...
namespace FenySoft.Core.Data
{
    ...
    public static class ValueToObjectsHelper
    {
        ...
        {
            Type[] types = TDataType.IsPrimitiveType(item.Type) ? new Type[] { item.Type } : DataTypeUtils.GetPublicMembers(item.Type, membersOrder).Select(x => x.GetPropertyOrFieldType()).ToArray();
