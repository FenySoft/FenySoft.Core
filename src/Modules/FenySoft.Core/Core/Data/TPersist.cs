...
{
    public class TPersist<T> : ITPersist<T>
    {
        ...
        public TPersist(Func<Type, MemberInfo, int> membersOrder = null, AllowNull allowNull = AllowNull.None)
        {
            Type = typeof(T);
    ...
    public class Persist : ITPersist<object>
    {
        ...
        #region ITPersist<object> Members
    ...
    //public class PersistTick : ITPersist<Tick>
    //{
    ...
    public static class PersistHelper
    {
        public static Expression CreateWriteBody(Expression item, Expression writer, Func<Type, MemberInfo, int> membersOrder, AllowNull allowNull)
        {
            ...
            if (TDataType.IsPrimitiveType(item.Type) || item.Type.IsEnum || item.Type == typeof(Guid) || item.Type.IsKeyValuePair() || item.Type.IsArray || item.Type.IsList() || item.Type.IsDictionary() || item.Type.IsNullable())
        ...
        private static Expression BuildWrite(Expression item, Expression writer, Func<Type, MemberInfo, int> membersOrder, AllowNull allowNull, bool isTop)
        {
            ...
            {
                if (!TDataType.IsPrimitiveType(type.GetGenericArguments()[0]) && !type.GetGenericArguments()[0].IsEnum && type != typeof(Guid))
            ...
            if (type.IsClass || type.IsStruct())
            {
                ...
                {
                    if (TDataType.IsPrimitiveType(type) || type.IsKeyValuePair())
                        list.Add(BuildWrite(Expression.PropertyOrField(item, member.Name), writer, membersOrder, allowNull, false));
                    else
                    {
        ...
        private static Expression WriteAssignedOrCurrentVariable(Expression variable, Expression writer, Func<Type, MemberInfo, int> membersOrder, AllowNull allowNull)
        {
            ...
            if (!TDataType.IsPrimitiveType(type) && !type.IsEnum && type != typeof(Guid))
            {
        ...
        {
            Debug.Assert(TDataType.IsPrimitiveType(item.Type));
        ...
        public static Expression CreateReadBody(Expression reader, Type itemType, Func<Type, MemberInfo, int> membersOrder, AllowNull allowNull)
        {
            ...
            if (TDataType.IsPrimitiveType(itemType) || itemType.IsEnum || itemType == typeof(Guid) || itemType.IsKeyValuePair() || itemType.IsArray || itemType.IsList() || itemType.IsDictionary() || itemType.IsNullable())
                return BuildRead(reader, itemType, membersOrder, allowNull, true);
            else
            {
                list.Add(Expression.Assign(item, Expression.New(itemType)));

                foreach (var member in TDataTypeUtils.GetPublicMembers(itemType, membersOrder))
        ...
        private static Expression BuildRead(Expression reader, Type itemType, Func<Type, MemberInfo, int> membersOrder, AllowNull allowNull, bool isTop)
        {
            ...
            if (TDataType.IsPrimitiveType(itemType))
        ...
        {
            Debug.Assert(TDataType.IsPrimitiveType(itemType));
