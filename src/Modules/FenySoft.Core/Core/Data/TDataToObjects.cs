...
{
    public class TDataToObjects : ITToObjects<ITData>
    {
        ...
        public TDataToObjects(Type type, Func<Type, MemberInfo, int> membersOrder = null)
        {
            if (!TDataType.IsPrimitiveType(type) && !type.HasDefaultConstructor())
                throw new NotSupportedException("No default constructor.");

            bool isSupported = TDataTypeUtils.IsAllPrimitive(type);
            if (!isSupported)
        ...
        public Expression<Func<object[], ITData>> CreateFromMethod()
        {
            ...
            var data = Expression.Variable(typeof(TData<>).MakeGenericType(Type));
