...
{
    public class TValueToString<T> : ITToString<T>
    {
        ...
        public TValueToString(int stringBuilderCapacity, IFormatProvider[] providers, char[] delimiters, Func<Type, MemberInfo, int> membersOrder = null)
        {
            ...
            var countOfType = TDataType.IsPrimitiveType(typeof(T)) ? 1 : TDataTypeUtils.GetPublicMembers(typeof(T), membersOrder).Count();
        ...
        public TValueToString(int stringBuilderCapacity, char[] delimiters, Func<Type, MemberInfo, int> membersOrder = null)
            : this(stringBuilderCapacity, ValueToStringHelper.GetDefaultProviders(typeof(T), membersOrder), delimiters, membersOrder)
        {
        }

        public TValueToString(Func<Type, MemberInfo, int> membersOrder = null)
    ...
    public static class ValueToStringHelper
    {
        public static Expression CreateToStringBody(Expression item, int stringBuilderCapacity, IFormatProvider[] providers, char delimiter, Func<Type, MemberInfo, int> membersOrder)
        {
            ...
            if (TDataType.IsPrimitiveType(item.Type) || TDataTypeUtils.GetPublicMembers(item.Type, membersOrder).Count() == 1)
            {
                var member = TDataType.IsPrimitiveType(item.Type) ? item : Expression.PropertyOrField(item, TDataTypeUtils.GetPublicMembers(item.Type, membersOrder).First().Name);
            ...
            foreach (var member in TDataTypeUtils.GetPublicMembers(item.Type, membersOrder))
            {
                ...
                if (i < TDataTypeUtils.GetPublicMembers(item.Type, membersOrder).Count() - 1)
