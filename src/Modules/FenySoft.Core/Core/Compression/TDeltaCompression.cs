...
using FenySoft.Core.Buffers;
using FenySoft.Core.Comparers;

namespace FenySoft.Core.Compression
{
    public static class TDeltaCompression
    {
        ...
        public static void Compress(BinaryWriter writer, long[] values, int index, int count, Helper helper)
        {
            ...
            TCountCompression.Serialize(writer, (ulong)value);
            if (count == 1)
                return;
            ...
            int bitCount = TBitUtils.GetBitBounds(maxDelta);

            bool writeSign = helper.Type == HelperType.Delta;

            TCommonArray common = new TCommonArray();
            ...
            TCountCompression.Serialize(writer, (ulong)bytesCount);
        ...
        public static void Decompress(BinaryReader reader, Action<int, long> values, int count)
        {
            ...
            long value = (long)TCountCompression.Deserialize(reader);
            values(index,value);
            if (count == 1)
                return;
            ...
            TCommonArray common = new TCommonArray();
            int bytesCount = (int)TCountCompression.Deserialize(reader);
        ...
        public class Helper
        {
            ...
            public void Serialize(BinaryWriter writer)
            {
                ...
                {
                    TCountCompression.Serialize(writer, Delta);
                    return;
                }
            ...
            public static Helper Deserialize(BinaryReader reader)
            {
                ...
                if (helper.Type == HelperType.OneStep)
                {
                    helper.Delta = TCountCompression.Deserialize(reader);
                    return helper;
                }
            ...
            public void Add(long value)
            {
                ...
                if (Count > 1)
                {
                    ...
                    int bits = TBitUtils.GetBitBounds(delta);
                    map[bits]++;
                    if (bits > maxIndex)
                        maxIndex = bits;
            ...
            private void SecondAdd(long value)
            {
                ...
                maxIndex = TBitUtils.GetBitBounds(Delta);
                map[maxIndex]++;
            }
