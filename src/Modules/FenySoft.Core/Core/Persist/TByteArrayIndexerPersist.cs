...
{
    public class TByteArrayIndexerPersist : ITIndexerPersist<Byte[]>
    {
        ...
        public void Store(BinaryWriter writer, Func<int, byte[]> values, int count)
        {
            ...
            for (int i = index; i < count; i++)
            {
                ...
                {
                    TCountCompression.Serialize(writer, (ulong)(value.Length + 1));
                    writer.Write(value);
                }
            }
        }
        ...
        public void Load(BinaryReader reader, Action<int, byte[]> values, int count)
        {
            if (reader.ReadByte() != VERSION)
                throw new Exception("Invalid TByteArrayIndexerPersist version.");
            ...
            for (int i = index; i < count; i++)
            {
                int length = (int)TCountCompression.Deserialize(reader);
