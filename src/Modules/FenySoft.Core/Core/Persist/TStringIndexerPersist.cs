...
{
    public class TStringIndexerPersist : ITIndexerPersist<String>
    {
                ...
                case PersistMode.Dictionary:
                    {
                        ...
                        new TInt32IndexerPersist().Store(writer, (idx) => { return indexes[idx]; }, count);
                    }
                    break;
        ...
        public void Load(BinaryReader reader, Action<int, string> values, int count)
        {
            if (reader.ReadByte() != VERSION)
                throw new Exception("Invalid TStringIndexerPersist version.");
                ...
                case PersistMode.Dictionary:
                    {
                        ...
                        new TInt32IndexerPersist().Load(reader, (idx, value) => { values(idx, value == NULL_ID ? null : map[value]); }, count);
                    }
                    break;
